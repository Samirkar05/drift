...
from src.modeling import ImageClassifier, TaskAdapter, AdapterImageClassifier
...
def custom_evaluate_adapter(
    ...
    info = vars(args)
    classification_head = get_classification_head(args, train_dataset_name, drift=False)

    if use_adapter_head:
        if isinstance(trained_adapter_source, str):
            trained_adapter = torch.load(trained_adapter_source)
        else:
            trained_adapter = trained_adapter_source

        if not isinstance(trained_adapter, TaskAdapter):
            raise TypeError(
                "Expected a TaskAdapter checkpoint for adapter evaluation."
            )
        model = AdapterImageClassifier(image_encoder, trained_adapter, classification_head)
    else:
        model = ImageClassifier(image_encoder, classification_head)
