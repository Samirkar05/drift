    ...
    def apply_to(self, pretrained_checkpoint, scaling_coef=1.0):
        ...
        with torch.no_grad():
            ...
            for key in pretrained_state_dict:
                ...
                vector_tensor = self.vector[key]

                # Align task vector tensor with pretrained tensor before merging.
                if vector_tensor.device != base_tensor.device:
                    vector_tensor = vector_tensor.to(base_tensor.device)
                if vector_tensor.dtype != base_tensor.dtype:
                    vector_tensor = vector_tensor.to(base_tensor.dtype)
